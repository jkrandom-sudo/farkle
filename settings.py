...
DEFAULT_PATH = Path.home() / ".farkle_settings.json"
...
def cycle_target(s: dict) -> dict:
    cur = s.get("target", 5000)
    idx = VALID_TARGETS.index(cur) if cur in VALID_TARGETS else 1
    s["target"] = VALID_TARGETS[(idx + 1) % len(VALID_TARGETS)]
    return s
